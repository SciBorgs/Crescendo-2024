...
public class Climbing {
    ...
    /**
     * @param coordinates current coordinates of the robot as a Translation2d supplier in meters as units.
     */
    public Rotation2d findChainAngle(Translation2d coordinates) {
        Translation2d nearest;
        if (Constants.alliance() == Alliance.Blue) {
            nearest = coordinates.nearest(List.of(BLUE_STAGE_AMPSIDE, BLUE_STAGE_SOURCESIDE, BLUE_STAGE_SPEAKERSIDE));
        }
        nearest = coordinates.nearest(List.of(RED_STAGE_AMPSIDE, RED_STAGE_SOURCESIDE, RED_STAGE_SPEAKERSIDE));
        ...
        if (nearest == BLUE_STAGE_AMPSIDE) {
            ...
            throw new NoSuchElementException("Oops. That's not supposed to happen. Climbing did an oopsie");
        }
    }
