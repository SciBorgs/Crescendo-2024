...
public class LedStrip extends SubsystemBase implements Logged, AutoCloseable {
  ...
  public static enum LEDTheme {
    ...
    SCIBORGS(() -> setAlternatingColor(Color.kDarkGray, Color.kYellow)), // Yellow 50%, Dark Grey 50%
    FEMAIDENS(() -> setAlternatingColor(Color.kPurple, Color.kLime)), // Yellow 50%, Green 50%
    BXSCIFLASH(() -> setMovingColor(Color.kGreen,Color.kYellow,5)), // Yellow ??%, Green ??%, moving
    GREEN(() -> setSolidColor(Color.kGreen)),
    RED(() -> setSolidColor(Color.kRed)),
    AUTO(() -> setMovingColor(Color.kBlack,Color.kYellow,3)), // Yellow Green 33%, Green 33%, Gold 33% , moving
    LIT(LedStrip::setFire), // Suppose to look like fire
    CHASE(() -> setMovingColor(Color.kDeepSkyBlue,Color.kDarkMagenta,6)), // Looks like those store lights chasing eachother in a loop
  ...
  public static String getBufferDataString(AddressableLEDBuffer ledBuffer) {
    ...
    for (int i = 0; i < ledBuffer.getLength(); i++) {
      ledBufferData[i] = ledBuffer.getLED(i).toHexString(); // it is a hex number but it spits it out
    }
  ...
  private static AddressableLEDBuffer setAlternatingColor(Color color1, Color color2) {
    final AddressableLEDBuffer ledBuffer = new AddressableLEDBuffer(LEDLENGTH);
    for (int i = 0; i < ledBuffer.getLength(); i++) {
      if (i % 2 == 0) {ledBuffer.setLED(i, color1);} else {ledBuffer.setLED(i, color2);}
    }
    return ledBuffer;
  }
  ...
  private static AddressableLEDBuffer setFire() {
    AddressableLEDBuffer ledBuffer = new AddressableLEDBuffer(LEDLENGTH); // the robot is lit! (but it should not burning)
    for (int i = 0; i < ledBuffer.getLength(); i++) {
      temp = (i + tick) % 5;
      if (temp < 1) {
