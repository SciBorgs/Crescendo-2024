...
public class Leds extends SubsystemBase implements Logged, AutoCloseable{
    ...
    public static enum LEDTheme{
        ...
        BXSCIFLASH,     //Yellow 20%, Green 80%, Moving, (All Yellow moves in one direction, yellow every 5 LEDs, green rest)
        ...
        EXPLODE         //Red 100%                  //means error now, unless you want to implement the ability for the robot to explode (potentially high risk high reward strategy)
    }
    ...
    public Leds(){
        setLEDTheme(LEDTheme.AUTO); //put default on initialization here (only run once though, so moving themes will not move)
    }
