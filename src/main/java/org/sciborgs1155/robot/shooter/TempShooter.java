...
public class TempShooter extends SubsystemBase implements AutoCloseable, Logged {
  ...
  public static TempShooter create() {
    return Robot.isReal()
        ? new TempShooter(new RealWheel(TOP_MOTOR), new RealWheel(BOTTOM_MOTOR))
        : new TempShooter(new SimWheel(), new SimWheel());
  }

  public static TempShooter none() {
    return new TempShooter(new NoWheel(), new NoWheel());
  }

  public TempShooter(WheelIO top, WheelIO bottom) {
    this.top = top;
    this.bottom = bottom;
