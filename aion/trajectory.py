...
class Trajectory:
    ...
    def get_optimal_settings(self):
        ...
        t_1 = (self.posY) / (
            (initial_v + delta_v)
            * casadi.cos(initial_theta + delta_theta)
            * np.sin(heading)
        )
        t_2 = (self.posY + speaker_width) / (
            (initial_v + delta_v)
            * casadi.cos(initial_theta + delta_theta)
            * np.sin(heading)
        )
        ...
        optimizer.subject_to(
            ...
            > ((initial_v + delta_v) * casadi.sin(initial_theta + delta_theta) * t_1)
            - ((1 / 2) * g * t_1**2)
        )

        optimizer.subject_to(
            min_speaker_height + (note_thickness / 2)
            < ((initial_v + delta_v) * casadi.sin(initial_theta + delta_theta) * t_2)
            - ((1 / 2) * g * t_2**2)
        )

        optimizer.subject_to(0 < delta_v + initial_v)
        optimizer.subject_to(delta_v + initial_v <= max_launch_speed)

        optimizer.subject_to(0 < delta_theta + initial_theta)

        optimizer.subject_to(delta_theta + initial_theta < np.pi / 2)
