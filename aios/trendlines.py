...
class TrajectoryFunctions:
    ...
    def fV(self, params, x, y):
        a, b, c = params
        return (a * x) + (b * y)  + c

    def fTheta(self, params, x, y):
        a, b, c = params
        return (a * x ** 2) + (b * y ** 2)  + c

    # Define the objective function (residuals)
    def objectiveTheta(self, params, x, y, z):
        return self.fTheta(params, x, y) - z

    # Define the objective function (residuals)
    def objectiveV(self, params, x, y, z):
        return self.fV(params, x, y) - z
    ...
    def returnFunctions(self):
        ...
        print(optimized_paramsv, optimized_paramst)
        os.system('clear')
        print(f"v(x, y) = {returnAv}x + {returnBv}y + {returnCv}")
        print(f"theta(x, y) = {returnAt}x^2 + {returnBt}y^2 + {returnCt}")

functions = TrajectoryFunctions(1000)
